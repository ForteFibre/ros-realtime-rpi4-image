...
CMAKE_TOOLCHAIN_FILE = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "data", "toolchain.cmake"
)
...
class Builder(object):
    def __init__(
        ...
        self.session_loop_device_file = os.path.join(
            self.cache_dir, LOOP_DEVICE_FILENAME
        )
        ...
        for profile_dir in profile_dirs:
            if not os.path.isdir(profile_dir):
                builtin_profile_dir = os.path.join(
                    os.path.abspath(os.path.dirname(__file__)), "data", profile_dir
                )
            ...
            # Merge configuration
            build_vars, env_vars = self._parse_config(
                os.path.join(profile_dir, "config.ini")
            )
            ...
            loop_device_setup_path = os.path.join(
                profile_dir, "scripts", "loop-device-setup"
            )
        ...
        self.cached_download_path = os.path.join(
            self.cache_dir, os.path.basename(self.build_vars["image_url"])
        )
        self.output_filename = os.path.join(
            self.out_dir, self.build_vars["output_filename"]
        )
    ...
    def verify_build_can_proceed(self):
        ...
        if self.extract_image_path is None:
            raise RequirementNotMetError(
                "the extract-image script must be defined for one of the profiles"
            )

        if self.loop_device_setup_path is None:
            raise RequirementNotMetError(
                "the loop-device-setup script must be defined for one of the profiles"
            )
        ...
        for var in required_variables:
            if var not in self.build_vars:
                raise RequirementNotMetError(
                    f"variable {var} is not defined in any profiles but is required"
                )
        ...
        for command in required_commands:
            if shutil.which(command) is None:
                raise RequirementNotMetError(
                    f"command {command} is not found on the host system but is required"
                )
    ...
    def download_and_extract_image_if_necessary(self):
        if not os.path.isfile(self.cached_download_path):
            ...
            self._run_script_on_host(
                [
                    ...
                    "dotbytes=10M",
                    "-O",
                    self.cached_download_path,
                    self.build_vars["image_url"],
                ]
            )
        else:
        ...
        self.logger.info(
            f"extracting {os.path.basename(self.cached_download_path)} into {self.output_filename}"
        )
        # Writing the code as a shell with pv allows us to get a better progress
        # bar than implementing this code directly in Python.
        self._run_script_on_host(
            f"{self.extract_image_path} {self.cached_download_path} | pv > {self.output_filename}",
            shell=True,
        )

    def setup_loop_device_and_mount_partitions(self):
        self.logger.info(
            f"expanding image to {self.build_vars['image_size']} with truncate"
        )
        self._run_script_on_host(
            ["truncate", "-s", self.build_vars["image_size"], self.output_filename]
        )

        partition_end_in_mb = int(
            round(os.path.getsize(self.output_filename) / 1000.0 / 1000.0)
        )
        partitions = (
            subprocess.check_output(["partx", "-g", self.output_filename])
            .decode("utf-8")
            .strip()
            .splitlines()
        )
        self.logger.info(f"partitions found: {partitions}")
        partition_num = len(partitions)
        self.logger.info(
            f"growing the last partition (partition number={partition_num}) to {partition_end_in_mb}MB"
        )
        ...
        self._run_script_on_host(
            [
                ...
                "resizepart",
                str(partition_num),
                str(partition_end_in_mb),
            ]
        )

        loop_device = (
            subprocess.check_output(
                ["losetup", "-P", "--show", "-f", self.output_filename]
            )
            .decode("utf-8")
            .strip()
        )
    ...
    def prepare_chroot(self):
        ...
        for i, mount_point in reversed(
            list(enumerate(self.build_vars["image_mounts"].split(",")))
        ):
            i += 1
        ...
        shutil.copy(
            ...
            os.path.join(
                self.chroot_path, self.build_vars["qemu_user_static_path"].lstrip("/")
            ),
        )

        shutil.copy(
            "/etc/resolv.conf", os.path.join(self.chroot_path, "etc", "resolv.conf")
        )
    ...
    def run_phase2_host_scripts(self):
        for phase2_host_path in self.phase2_host_paths:
            self._run_script_on_host(
                phase2_host_path,
                more_env_vars={"CMAKE_TOOLCHAIN_FILE": CMAKE_TOOLCHAIN_FILE},
            )
    ...
    def _check_pause(self, step: str):
        if self.pause_after == step:
            self.logger.warn(
                f"pausing after {step} as it is configured via the build var pause_after"
            )
    ...
    def _run_script_on_host(
        self, args: Sequence[str] | str, shell: bool = False, more_env_vars: dict = {}
    ):
